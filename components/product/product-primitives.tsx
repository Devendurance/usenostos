...
export function DataPanel({ title, description, children, className = "", aside }: { title: string; description?: string; children: ReactNode; className?: string; aside?: ReactNode }) {
  return (
    <section className={`rounded-[var(--radius-card)] border border-[var(--line)] bg-white p-5 md:p-6 ${className}`}>
...
export function Metric({ label, value = "—", hint }: { label: string; value?: string; hint?: string }) {
  return (
    <div className="min-h-28 rounded-[var(--radius-control)] border border-[var(--line)] bg-[#fbfaf8] p-4">
...
export function StateNotice({ title, message, tone = "neutral" }: { title: string; message: string; tone?: "neutral" | "warning" }) {
  return (
    <div className={`flex gap-3 rounded-[var(--radius-control)] border p-4 ${tone === "warning" ? "border-[var(--ink)] bg-[var(--sticky-yellow)]/35" : "border-[var(--line)] bg-[#fbfaf8]"}`} role="status">
...
export function TableEmpty({ columns, title, message }: { columns: string[]; title: string; message: string }) {
  return (
    <div className="overflow-x-auto rounded-[var(--radius-control)] border border-[var(--line)]">
...
export function FieldShell({ label, value = "—", hint }: { label: string; value?: string; hint?: string }) {
  return <div><p className="text-xs font-semibold uppercase tracking-[.1em] text-[var(--muted)]">{label}</p><p className="mt-2 min-h-12 rounded-[var(--radius-control)] border border-[var(--line)] bg-[#fbfaf8] px-4 py-3 text-sm font-semibold tabular">{value}</p>{hint && <p className="mt-2 text-xs leading-5 text-[var(--muted)]">{hint}</p>}</div>;
}
