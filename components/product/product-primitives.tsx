...
export function StateNotice({ title, message, tone = "neutral" }: { title: string; message: string; tone?: "neutral" | "warning" }) {
  return (
    <div className={`flex gap-3 rounded-control border p-4 ${tone === "warning" ? "border-[var(--ink)] bg-[var(--sticky-yellow)]/35" : "border-[var(--line)] bg-[#fbfaf8]"}`} role="status">
      ...
      <div className="min-w-0"><p className="text-sm font-semibold">{title}</p><p className="mt-1 break-words text-sm leading-6 text-muted-foreground">{message}</p></div>
    </div>
  );
}
...
export function DefinitionRows({ rows }: { rows: Array<{ label: string; value?: ReactNode }> }) {
  return (
    <dl className="divide-y divide-[var(--line)] border-y border-[var(--line)]">
      {rows.map((row) => <div key={row.label} className="grid gap-1 py-4 sm:grid-cols-[minmax(140px,0.7fr)_1.3fr] sm:gap-5"><dt className="text-xs font-semibold uppercase tracking-[.12em] text-muted-foreground">{row.label}</dt><dd className="text-sm font-semibold tabular">{row.value ?? "—"}</dd></div>)}
    </dl>
  );
}
  ...
  return (
    <div className="overflow-x-auto rounded-control border border-[var(--line)]">
      <table className="w-full min-w-[640px] border-collapse text-left text-sm">
        <thead className="bg-[#fbfaf8]"><tr>{columns.map((column) => <th key={column} scope="col" className="border-b border-[var(--line)] px-4 py-3 text-xs font-semibold uppercase tracking-[.1em] text-muted-foreground">{column}</th>)}</tr></thead>
        <tbody><tr><td colSpan={columns.length} className="px-6 py-14 text-center"><p className="font-semibold">{title}</p><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted-foreground">{message}</p></td></tr></tbody>
      </table>
    </div>
  );
}
...
export function FieldShell({ label, value = "—", hint }: { label: string; value?: string; hint?: string }) {
  return <div><p className="text-xs font-semibold uppercase tracking-[.1em] text-muted-foreground">{label}</p><p className="mt-2 min-h-12 rounded-control border border-[var(--line)] bg-[#fbfaf8] px-4 py-3 text-sm font-semibold tabular">{value}</p>{hint && <p className="mt-2 text-xs leading-5 text-muted-foreground">{hint}</p>}</div>;
}
