...
- P5 Nostos InstantPool (uncommitted 2026-08-19): protocol-owned `NostosInstantPool.sol` buys PENDING P4 tickets at a deterministic bps discount (base+utilization+size slopes, min/max clamps, maxUtilization), pays real Testnet USDT immediately, holds the claim, harvests full settlement via P4 `claimRedeem`. Roles: DEFAULT_ADMIN (pricing+withdraw), MANAGER (fund), PAUSER (pause); harvest permissionless and not paused. Tooling behind `P5_ENABLE_TESTNET_DEPLOY=true` (chain 968 only); no pool deployed; `bot-testnet.json` has no `p5` key. Design: `docs/superpowers/specs/2026-08-19-p5-nostos-instant-pool-core-design.md`; Plan: `docs/superpowers/plans/2026-08-19-p5-nostos-instant-pool-core.md`.
