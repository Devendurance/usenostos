...
- P4 transferable redemption claim tickets (committed in `4673041` / `f6dcd1f`): `NostosAsyncVaultP4` + `NostosRedemptionTicket` (ERC-721). Ticket id == request id; request status enum None/Pending/Claimable/Claimed; PENDING face value = `vault.sharesToAssets(request.shares)` (never `assetsClaimable`, which is 0 while PENDING). P4 Vault `0xd6333420629dcd2f9bce31ef12c8a95e5e13fdac`, Ticket `0x59b2580c5bdbcfb32066ed013f9ff0f7a812fd7f` (testnet, persisted in `contracts/addresses/bot-testnet.json`).
- P5 Nostos InstantPool (deployed Testnet 2026-08-19): protocol-owned `NostosInstantPool` at `0x2f18e935ca51729777503d4dba866a339f284472`. Buys PENDING P4 tickets at a deterministic bps discount, pays USDT immediately, permissionless harvest. Persisted under `p5.instantPool`.
- P6 Public LP InstantPool (implemented 2026-08-19, not deployed): `NostosInstantPoolP6` — permissionless USDT deposits mint non-transferable 18-dec `nLP` shares. NAV = availableLiquidity + outstandingCostBasis; availableLiquidity excludes `accruedProtocolFees`. Virtual offset 1e6 assets / 1e18 shares. Fixed 24h cooldown reset on every deposit. Harvest: 10% of realized spread → accrued protocol fees (treasury claim), 90% → LP NAV. Quotes/purchases use availableLiquidity. No `fund`/`withdrawLiquidity`. Tooling: `P6_ENABLE_TESTNET_DEPLOY=true` + `P6_PROTOCOL_TREASURY`. Frontend `/pool` prefers P6 when `p6.instantPool` exists, else P5.
