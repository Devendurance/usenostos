...
import { vaultAbi, vaultBytecode } from "@/scripts/registry/artifact";
...
async function main() {
  ...
  if (!plan.enabled) {
    console.log(`VAULT DEPLOY DISABLED: ${plan.reason}`);
    process.exit(0);
  }
  if (!plan.ok) {
    console.error(`VAULT DEPLOY REFUSED: ${plan.reason}`);
    process.exit(1);
  }
  ...
  console.log("DEPLOYING NostosAsyncVault");
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        ...current,
        asyncVault: vaultAddress,
        asyncVaultTx: hash,
        asyncVaultBlock: String(receipt.blockNumber),
        asyncVaultDeployedAt: new Date().toISOString(),
      },
      null,
      2,
    ),
...
main().catch((err) => {
  console.error("VAULT DEPLOY FAILED:", err instanceof Error ? err.message : err);
  process.exit(1);
});